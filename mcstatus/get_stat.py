...
async def motd(address: str, full=False):
    ...
    if not address:
        if full:
            return MessageSegment.text('[MCJE服务器信息]\n'
                                       '正确用法:\n'
                                       'motdfull <服务器IP> (电脑版)\n'
                                       'motdpe <服务器IP> (手机版)')
        else:
    ...
    try:
        ret = mc.MinecraftServer.lookup(address)
    ...
    return MessageSegment.text(f'Hana帮你查询到这些~:\n') + \
           pic_text + \
           MessageSegment.text(f'服务端名：{ret["version"]["name"]}\n'
...
async def motdpe(address: str):
    address = address.strip()
    if not address:
        return MessageSegment.text('[MCBE服务器信息]\n'
                                   '正确用法:\n'
                                   'motd <服务器IP> (电脑版)\n'
                                   'motdpe <服务器IP> (手机版)')
    if address.rfind(':') == -1:
    ...
    try:
        ret = mc.MinecraftBedrockServer.lookup(address)
        ret = await ret.async_status()
    except Exception as e:
        nonebot.logger.opt(colors=True, exception=e).error('')
    ...
    return MessageSegment.text('Hana帮你查询到这些~:\n'
                               f'协议版本：{ret.version.protocol}\n'
